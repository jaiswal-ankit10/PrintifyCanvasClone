    ...
    const loadContent = async () => {
      try {
        ...
        if (sideJson) {
          if (!isMounted) return;
          try {
            console.log("Loading Side JSON into Preview:", sideJson); // Log the JSON
            await canvas.loadFromJSON(sideJson);
            console.log("Loaded Objects:", canvas.getObjects().length); // Log loaded count
            ...
            if (sourceWidth && sourceHeight) {
                ...
                canvas.getObjects().forEach(obj => {
                    ...
                    const isPrintGuide = 
                        obj.name === "printGuide" || 
                        obj.name === "guideline" ||
                        (obj.type.toLowerCase() === "rect" && obj.strokeDashArray && obj.strokeDashArray.length > 0 && !obj.fill);
                    ...
                    obj.left = previewCenter.x + (relX * scaleFactor) + (offsetX * (large ? 1 : 0.2)); 
                    // Note: If offsetX is pixels in 600px preview, we apply it directly.
                    // If large=false (140px), we should scale the offset too.
                    
                    obj.top = previewCenter.y + (relY * scaleFactor) + (offsetY * (large ? 1 : 0.2));
